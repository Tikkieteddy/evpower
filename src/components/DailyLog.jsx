    ...
    { key: "note", label: "หมายเหตุ", render: (row) => row.note || "-" },
    {
      ...
      render: (row) => (
        <div className="flex gap-2">
          <button
            className="grid h-9 w-9 place-items-center rounded-lg border border-slate-200 text-blue-700 hover:bg-blue-50"
    ...
    <div className="grid gap-5">
      <form onSubmit={submit} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
          <div>
            <h2 className="text-lg font-bold text-slate-950">{editingId ? "แก้ไขรายการเดินทาง" : "บันทึกรายวัน"}</h2>
            <p className="text-sm text-slate-500">กรอกระยะทางทุกวัน และเพิ่มข้อมูลชาร์จเฉพาะวันที่มีการชาร์จ</p>
          </div>
          <button
            type="button"
            onClick={onResetSample}
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 font-semibold text-slate-700 hover:bg-slate-50"
          >
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label className="mt-6 inline-flex min-h-11 items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 font-semibold text-slate-700">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <button className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg bg-blue-600 px-4 font-bold text-white shadow-sm hover:bg-blue-700" type="submit">
            <Plus size={18} /> {editingId ? "บันทึกการแก้ไข" : "เพิ่มรายการ"}
          </button>
          {editingId ? (
            <button
              type="button"
              className="min-h-11 rounded-lg border border-slate-200 px-4 font-bold text-slate-700 hover:bg-slate-50"
              onClick={() => {
                setEditingId(null);
                setForm(blankForm);
              }}
